...
const DriverAssignmentModal: React.FC<DriverAssignmentModalProps> = ({
  ...
  const handleAssign = async () => {
    ...
    try {
      setLoading(true);
      const updatedOrder = await orderService.assignDriver(order._id, selectedDriverId, notes, vehicleNumber);
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* Vehicle Number */}
        <div>
          <label htmlFor="vehicle-number" className="block text-sm font-medium text-gray-700 mb-2">
            Vehicle Number
          </label>
          <input
            id="vehicle-number"
            ...
            className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm uppercase"
          />
        </div>
