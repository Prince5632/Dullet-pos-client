...
const transformOrderToDeliveryData = (order: Order) => {
  return {
    company: {
    ...
    order: {
      ...
      customer: {
        ...order.customer,
        customerId: order.customer?.customerId || `CUST${Date.now().toString().slice(-4)}`,
        __v: 0
      },
      godown: order.godown || {
        _id: "default",
...
const convertComponentToImage = async (deliveryData: any): Promise<string> => {
  ...
  return new Promise((resolve, reject) => {
    root.render(
      React.createElement('div', { className: 'delivery-invoice pdf-generation' },
      ...
      try {
        ...
        const canvas = await html2canvas(tempContainer, {
          ...
          onclone: (clonedDoc) => {
            ...
            const clonedContainer = clonedDoc.querySelector('.pdf-generation-container') as HTMLElement;
            if (clonedContainer) {
              clonedContainer.style.width = `${A4_WIDTH_PX}px`;
              clonedContainer.style.background = 'white';
              clonedContainer.style.padding = '15px';
              clonedContainer.style.fontFamily = '"DejaVu Sans", Arial, sans-serif';
              clonedContainer.style.overflow = 'hidden';
            }
            
            // Apply PDF-specific styles to the delivery invoice
            const deliveryInvoice = clonedDoc.querySelector('.delivery-invoice') as HTMLElement;
            if (deliveryInvoice) {
...
const convertImageToPDF = (imageData: string, fileName: string, downloadDirectly: boolean = true): Promise<Blob | void> => {
...
export const generateDeliveryNotePDF = async (order: Order, downloadDirectly: boolean = true): Promise<Blob | void> => {
  try {
    // Transform order data to match DeliveryInvoiceTemplate format
    const deliveryData = transformOrderToDeliveryData(order);

    // Step 1: Convert component to image
    console.log('Converting component to image...');
