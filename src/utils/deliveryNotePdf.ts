...
export const generateDeliveryNotePDF = (
  ...
  if (order.driverAssignment?.driver) {
    ...
    const deliveryDetails = [
      ...
      ...(order.driverAssignment.vehicleNumber
        ? [["Vehicle Number:", order.driverAssignment.vehicleNumber]]
        : []),
      ...(order.driverAssignment.assignedAt
        ? [
            [
              "Assigned At:",
              new Date(order.driverAssignment.assignedAt).toLocaleString(
                "en-IN"
              ),
            ],
          ]
      ...
      ...(order.driverAssignment.deliveryAt
        ? [
            [
              "Delivered At:",
              new Date(order.driverAssignment.deliveryAt).toLocaleString(
                "en-IN"
              ),
            ],
          ]
  ...
  if (
    ...
    if (order.driverAssignment?.driverNotes) {
      const driverText = doc.splitTextToSize(
        `Driver Notes: ${order.driverAssignment.driverNotes}`,
        pageWidth - 28
      );
