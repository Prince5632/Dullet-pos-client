...
import type {
  ApiResponse,
  PaginationResponse,
  UpdateProductionForm as MainUpdateProductionForm,
} from "../types";
...
class ProductionService {
  ...
  async getProductions(
    ...
    const url = queryString
      ? `${API_CONFIG.ENDPOINTS.PRODUCTIONS}?${queryString}`
      : API_CONFIG.ENDPOINTS.PRODUCTIONS;
  ...
  async getProductionById(id: string): Promise<ApiResponse<Production>> {
    return await apiService.get<Production>(
      API_CONFIG.ENDPOINTS.PRODUCTION_BY_ID(id)
    );
  }

  // Get production by batch ID
  async getProductionByBatchId(
    batchId: string
  ): Promise<ApiResponse<Production>> {
    return await apiService.get<Production>(
      API_CONFIG.ENDPOINTS.PRODUCTION_BY_BATCH_ID(batchId)
    );
  }

  // Create production
  async createProduction(
    productionData: CreateProductionForm
  ): Promise<ApiResponse<Production>> {
    ...
    return await apiService.post<Production>(
      API_CONFIG.ENDPOINTS.PRODUCTIONS,
      formData,
      {
        headers: {
  ...
  async updateProduction(
    id: string,
    productionData: UpdateProductionForm
  ): Promise<ApiResponse<Production>> {
    const formData = new FormData();
    ...
    return await apiService.put<Production>(
      API_CONFIG.ENDPOINTS.PRODUCTION_BY_ID(id),
      formData,
      {
        headers: {
  ...
  async getProductionStats(): Promise<ApiResponse<ProductionStats>> {
    return await apiService.get<ProductionStats>(
      API_CONFIG.ENDPOINTS.PRODUCTION_STATS
    );
  }

  // Get productions by location
  async getProductionsByLocation(
    location: string
  ): Promise<ApiResponse<{ productions: Production[] }>> {
    return await apiService.get<{ productions: Production[] }>(
      API_CONFIG.ENDPOINTS.PRODUCTION_BY_LOCATION(location)
    );
  }
...
export type {
  ...
  MainUpdateProductionForm as UpdateProductionForm,
  ProductionStats,
  ProductionFilters,
};
